...
class DQN_Othello(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.epsilon2 = epsilon2_init
        self.epsilon2_min = epsilon2_min
        self.epsilon2_delta = (epsilon2_init - epsilon2_min) / (run_step * explore2_ratio)
    ...
    def act(self, state, training=True):
        ...
        if np.random.random() < epsilon:
            if np.random.random() < self.epsilon2:
                return None
            else:
                batch_size = (
            ...
            action = torch.argmax(
                self.network(self.as_tensor(state)), -1, keepdim=True
            ).numpy()
    ...
    def epsilon_decay(self, delta_t):
        ...
        # new_epsilon2 = self.epsilon2 - delta_t * self.epsilon2_delta
        # self.epsilon2 = max(self.epsilon2_min, new_epsilon2)
        pass
