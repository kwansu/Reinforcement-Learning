...
class Avoid(BaseEnv):
    ...
    def render(self):
        ...
        for obj in self.falling_objs:
            if obj.is_active == False:
                continue
            sprite = self.star_sprite if obj.is_star else self.poop_sprite
            self.window.blit(sprite, 10 * (obj.pos - obj.half_size))
    ...
    def step(self, action):
        ...
        if self.step_count > self.falling_interval:
            self.step_count = 0
            self.star_not_fall_count += 1
            new_object = self.create_object()
            new_object.pos = [random.randrange(1, 28), -1]

            if self.star_not_fall_count > 2 and random.random() < 0.3:
                self.star_not_fall_count = 0
                new_object.is_star = True
            else:
                new_object.is_star = False
        ...
        for obj in self.falling_objs:
            ...
            if obj.pos[1] >= self.play_height:
                ...
                if obj.is_star:
                    state[:, :, int(obj.pos[1]), int(obj.pos[0])] = 4
                else:                    
                    state[:, :, int(obj.pos[1]), int(obj.pos[0])] = 2
